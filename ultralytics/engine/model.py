...
class Model(nn.Module):
    ...
    def __init__(
        ...
        """
        super().__init__()
        logging.warning(self.__class__.__mro__)  # 应该是 <class 'torch.nn.modules.module.Module'>
        self.callbacks = callbacks.get_default_callbacks()  # deepcopy了一堆函数名？
        ...
        logging.warning(str(model).strip())
    ...
    def _new(self, cfg: str, task=None, model=None, verbose=False) -> None:
        ...
        """
        cfg_dict = yaml_model_load(cfg)  # return: nc:80, scales:nsmxl dict, backbone:dict, head:dict,scale:m,yaml_file:path
        self.cfg = cfg
        self.task = task or guess_model_task(cfg_dict)  # 从head的倒数第一行的倒数第二个元素传达的
    ...
    def _smart_load(self, key: str):
        ...
        """
        try:
            return self.task_map[self.task][key]  # 本处的task_map属于方法被@property当作属性，动态的计算，直接调用无需定义实例化,而且因为传进来是YOLO的model的taskmap实现
