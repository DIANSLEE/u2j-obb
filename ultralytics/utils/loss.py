...
class RotatedBboxLoss(BboxLoss):
    ...
    def forward(self, pred_dist, pred_bboxes, anchor_points, target_bboxes, target_scores, target_scores_sum, fg_mask):
        ...
        weight = target_scores.sum(-1)[fg_mask].unsqueeze(-1)
        # # 验证target angle分布
        # t_angles = target_bboxes[fg_mask][..., 4].detach().cpu().numpy()
        # if len(t_angles) > 0:
        #     neg_pct = (t_angles < 0).sum() / len(t_angles) * 100
        #     with open(r'C:\Users\dians\Desktop\ultralytics-main\target_angle_log.txt', 'a') as f:
        #         f.write(
        #             f"target angle: total={len(t_angles)} neg={neg_pct:.1f}% min={t_angles.min():.2f} max={t_angles.max():.2f}\n")
        iou = probiou(pred_bboxes[fg_mask], target_bboxes[fg_mask])
        loss_iou = ((1.0 - iou) * weight).sum() / target_scores_sum
        # obb改动点
        # 单独的angle L1 loss
        pred_angle = pred_bboxes[fg_mask][..., 4:5]
        target_angle = target_bboxes[fg_mask][..., 4:5]
        diff = pred_angle - target_angle
        # 处理循环：把误差wrap到[-pi, pi]
        diff = (diff + math.pi) % (2 * math.pi) - math.pi
        loss_angle = diff.abs().mean() * 0.5
        loss_iou = loss_iou + loss_angle
...
class v8DetectionLoss:
    ...
    def __init__(self, model, tal_topk=10):  # model must be de-paralleled
        ...
        # # 添加改动Focal lss
        # from ultralytics.agd.AHFL import AdaptiveThresholdFocalLoss
        # g = 1
        # if g > 0:
...
class v8SegmentationLoss(v8DetectionLoss):
    ...
    def __call__(self, preds, batch):
        ...
        pred_distri, pred_scores = torch.cat([xi.view(feats[0].shape[0], self.no, -1) for xi in feats], 2).split(
            (self.reg_max * 4, self.nc), 1
        )  # 前边loss里设置了self.no=m.nc+self.reg_max*4 这里好像把这个解开了把feats(2,66,64,88)前面留着66=64+2，后面全加一起等于好几千64*88+32*44...

        # B, grids, ..
        pred_scores = pred_scores.permute(0, 2, 1).contiguous()  # 改变张量顺序？
        pred_distri = pred_distri.permute(0, 2, 1).contiguous()
        ...
        # Targets
        try:
            batch_idx = batch["batch_idx"].view(-1, 1)  # 哦哦这个batch_idx是不是哪些实例属于哪个图片？一共我这里设置了2所以000111111
        ...
        loss[0] *= self.hyp.box  # box gain 各种损失占比权重，关注点在谁更多
    ...
    def calculate_segmentation_loss(
        ...
        for i, single_i in enumerate(zip(fg_mask, target_gt_idx, pred_masks, proto, mxyxy, marea, masks)):
            ...
            if fg_mask_i.any():
                ...
                if overlap:
                    gt_mask = masks_i == (mask_idx + 1).view(-1, 1, 1)  # ground truth mask（gt_mask），并进行广播操作以比较 masks_i 和 mask_idx + 1,因为类别从1开始吧
...
class MultiTaskLoss(v8DetectionLoss):  # 改动点
