...
class OBB(Detect):
    ...
    def forward(self, x):
        ...
        # angle = angle.sigmoid() * math.pi / 2  # [0, pi/2] obb改动点
        # angle = (angle.sigmoid() - 0.5) * math.pi  # [-pi/4, 3pi/4]  # obb改动点
        # angle = angle.sigmoid() * math.pi / 2  # [0, pi/2]
        angle = (angle.sigmoid() - 0.5) * 2 * math.pi  # [-pi, pi] full360
...
class MultiTask(Detect):  # 改动点
    ...
    def forward(self, x):
        ...
        pred_kpt = self.pose_head.kpts_decode(bs, kpt, self.anchors, self.strides)  # 改动点未改动
